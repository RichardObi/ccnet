...
def get_file_transformations(file_name, phase, secondphase):
    transformed = [file_name,
                   file_name.replace(f"_{phase}", f"_{secondphase}"),
                   file_name.replace(f"_{secondphase}", f"_{phase}"),#]
                   file_name.replace(f"_mask", f"_slice")]
                   #file_name.replace(f"_{phase}", ""), # for segmentation masks
                   #file_name.replace(f"_{secondphase}", ""),]  # for segmentation masks
    final_transformed = []
    for transformed_filename in transformed:
        final_transformed.extend([transformed_filename, transformed_filename.replace("png", "jpg"), transformed_filename.replace("jpg", "png"), transformed_filename.replace("_synthetic.png", ".png"), transformed_filename.replace("_slice", "_mask"), transformed_filename.replace("_mask", "_slice"), transformed_filename.replace(".png", "_synthetic.png")])
    final_list = list(set(final_transformed)) # remove duplicates
    if 'Breast_MRI_009_0001_slice65' in file_name:
        print(f"file_name: {file_name}")
        print(f"final_list: {final_list}")
    return final_list # remove duplicates
